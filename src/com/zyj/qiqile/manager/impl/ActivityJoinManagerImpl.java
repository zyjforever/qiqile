...
public class ActivityJoinManagerImpl extends BasicManageImpl implements
	...
	@Override
	public TaskResult queryActivityJoinBOByActivityId(String activityId) {
		if(activityId==null)
		return null;
		
		Map<String, String> params = new HashMap<String, String>();
		params.put(ActivityJoinBO.ACTIVITY_ID, activityId);
		TaskResult result = new TaskResult();
		JSONObject jsonObject = null;
		try {
			jsonObject = httpConnectionHelper.sendPostDataAndResult(URL
			...
			try {
				...
				if (errorCode == ServerErrorConstants.CODE_SUCCESS) {
					...
					if (!jsonObject.isNull(ServerConstants.DATA))
						result.put(
								"activityJoinBOList",
								ActivityJoinConverter.jsonTOActivityJoinBOList(jsonObject
				...
				Log.e(TAG, "queryActivityJoinBOByActivityId", e);
				result.setResult(ResultCode.UNKNOW_ERROR);
				return result;
			}
		}
